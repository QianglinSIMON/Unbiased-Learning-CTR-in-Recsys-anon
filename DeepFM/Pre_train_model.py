...
#
# df_tuning_oversamples = smote_knn_oversample(df_tuning, sparse_features, dense_features,
#                                                      target='is_click', random_state=seed)
# len(df_tuning)/len(df_biased)

# 测试数据集加载器
test_label = pd.DataFrame(df_test[target])
test_data = df_test.drop(columns=target[0])

test_tensor_data = torch.utils.data.TensorDataset(torch.from_numpy(np.array(test_data)).float(),
                                                  torch.from_numpy(np.array(test_label)).float())
...
# 训练数据集加载器
train_label = pd.DataFrame(df_train[target])
train_data = df_train.drop(columns=target[0])

train_tensor_data = torch.utils.data.TensorDataset(torch.from_numpy(np.array(train_data)).float(),
                                                   torch.from_numpy(np.array(train_label)).float())
...
for epoch in range(epochs):
    ...
    for index, (x, y) in enumerate(train_loader):
        ...
        # 使用自定义损失函数
        loss = custom_loss_function(y_hat_train, y,
                                    model,
...
# # 在测试集上计算评估指标
# avg_test_loss, test_auc, precision, recall, f1, ndcg_at_k = get_metrics(test_loader, model, k=5, device=device)

# print('test loss: {:.3f}, test auc: {:.3f}, precision: {:.3f}, recall: {:.3f}, f1_score: {:.3f}, ndcg@5: {:.3f}'.format(
#             avg_test_loss, test_auc, precision, recall, f1, ndcg_at_k))

# mlx worker launch -- python3 Pre_train_model.py
