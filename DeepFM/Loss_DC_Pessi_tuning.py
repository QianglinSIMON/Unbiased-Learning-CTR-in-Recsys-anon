...
early_stopping = EarlyStopping(patience=patience, min_delta=0.001,
                               save_path="Loss_DC_Pessi_model_tuning_{}_percent.pth".format(
...
for epoch in range(epochs):
    ...
    for (x_train, y_train), (x_surrogate, y_surrogate) in zip(train_loader, surrogate_loader):
        ...
        var_bias_shift = torch.tensor(
            (1/len(mse_loss_train))*(var_mse_train ) +(1/len(mse_loss_surrogate))* (var_mse_surrogate )
        ...
        weight_doma = (1/len(mse_loss_train))*(var_mse_train ) +(1/len(mse_loss_surrogate))* (var_mse_surrogate ) + Upper_coef ** 2
        - 2*(covariance_value/(len(mse_loss_train)*len(mse_loss_surrogate)))
...
# 将最佳权重矩阵保存到文件
np.savetxt("DF_best_Pessi_tuning_{}_percent_alpha_matrix.txt".format(int(100 * tuning_percentage)),
           best_alpha_matrix,
           delimiter=",")

metrics_df.to_csv('metrics_trained_LOSS_DC_Pessi_tuning_{}_percent.csv'.format(int(100 * tuning_percentage)),
