...
tuning_percentage = 0.05  # 0.05,0.1,0.15,0.2,0.25

batch_size_train = batch_size_surrogate = 128 # 256，512, 1024
...
early_stopping = EarlyStopping(patience=patience, min_delta=0.001,
                               save_path="Loss_DC_NonPessi_model_tuning_{}_percent.pth".format(
...
for epoch in range(epochs):
    ...
    for (x_train, y_train), (x_surrogate, y_surrogate) in zip(train_loader, surrogate_loader):
        ...
        + (1 / len(mse_loss_surrogate)) * (var_mse_surrogate) 
        - 2 * (covariance_value / (len(mse_loss_train) * len(mse_loss_surrogate)))
...
# 将最佳权重矩阵保存到文件
np.savetxt("DF_best_Nonpessi_tuning_{}_percent_alpha_matrix.txt".format(int(100 * tuning_percentage)),
           best_alpha_matrix,
           delimiter=",")

metrics_df.to_csv('metrics_trained_LOSS_DC_Nonpessi_tuning_{}_percent.csv'.format(int(100 * tuning_percentage)),
