...
surrogate_loader = create_data_loaders(batch_size=batch_size_surrogate, dataset=df_tuning, sparse_features=sparse_features, dense_features=dense_features, target=target, shuffle=True)
...
# 初始化早停
early_stopping = EarlyStopping(patience=patience, min_delta=0.001, save_path="Loss_DC_Nonpessi_fintuning_model_{}_percent.pth".format(int(tuning_percentage*100)) )
...
for epoch in range(epochs):
    ...
    surrogate_sparse_inputs, surrogate_dense_inputs, surrogate_labels) in zip(train_loader_iter, surrogate_loader_iter):
       ...
       # 将 x_surrogate 和 y_surrogate 合并为一个数据集
        ...
        # 计算最优权重 best_alpha
        weight_frac = (1 / len(mse_loss_surrogate)) * (var_mse_surrogate) - (
                    covariance_value / (len(mse_loss_train) * len(mse_loss_surrogate)))
        weight_doma = (1 / len(mse_loss_train)) * (var_mse_train) + mean_shift ** 2
        + (1 / len(mse_loss_surrogate)) * (
    ...
    # 打印当前 epoch 的评估结果
    print(f'Epoch {epoch}/{epochs}: Train Loss: {avg_train_loss:.3f}, Test Loss: {avg_test_loss :.3f}, Val AUC: {val_auc:.3f},Test AUC: {test_auc:.3f}, Epoch_mean_best_alpha:{epoch_mean_best_alpha:.3f}, '
...
# 将最佳权重矩阵保存到文件
np.savetxt("best_Loss_DC_Nonpessi_fintuning_alpha_matrix_tuning_{}_percent.txt".format(int(tuning_percentage*100)), best_alpha_matrix, delimiter=",")

metrics_df.to_csv('metrics_trained_Loss_DC_Nonpessi_fintuning_{}_percent.csv'.format(int(tuning_percentage*100)), index=False) #依赖于微调数据比例
...
# mlx worker launch -- python3 Loss_DC_Nonpessi_tuning.py
