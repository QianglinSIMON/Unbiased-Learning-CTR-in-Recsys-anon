...
df_train=pd.concat([df_biased,df_tuning],axis=0)
...
train_loader=create_data_loaders(batch_size=tuning_batch_size, dataset=df_train, sparse_features=sparse_features, dense_features=dense_features, target=target, shuffle=True)
...
# 初始化早停
early_stopping = EarlyStopping(patience=patience, min_delta=0.001, save_path="supervised_all_model_tuning_{}_percentage.pth".format(int(tuning_percentage*100)))
...
for epoch in range(epochs):
    ...
    print(f'epoch/epochs: {epoch }/{epochs}, train loss: {avg_train_loss :.3f}, test loss: {avg_test_loss :.3f}, val auc: {auc_val:.3f}, test auc: {auc:.3f}, precision: {precision:.3f}, recall: {recall:.3f}, f1_score: {f1:.3f}, ndcg@5: {ndcg_at_k:.3f}')
...
file_name = 'metrics_supervised_all_tuning_{}_percentage.csv'.format(int(tuning_percentage*100)) ## 依赖于微调数据比例！
...
# mlx worker launch -- python3 Supervised_all.py
