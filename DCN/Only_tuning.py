...
tuning_loader=create_data_loaders(batch_size=tuning_batch_size, dataset=df_tuning, sparse_features=sparse_features, dense_features=dense_features, target=target, shuffle=True)
...
# 初始化早停
early_stopping = EarlyStopping(patience=patience, min_delta=0.001, save_path="only_tuning_{}_percentage.pth".format(int(tuning_percentage*100)))
...
for epoch in range(epochs):
    ...
    model.train()
    for sparse_inputs, dense_inputs, labels in tuning_loader:
...
file_name = 'metrics_Only_tuning_{}_percentage.csv'.format(int(tuning_percentage*100)) ## 依赖于微调数据比例！
...
# mlx worker launch -- python3 Only_tuning.py
