...
def Grid_serch_alpha_CV(alpha_values, tuning_batch_size, df_unbiased, df_biased,
                        ...
                        embedding_index,embedding_size,
              dense_feature_num,cross_layer_num,
              dnn_hidden_units,
              dropout,
              l2_reg_embedding, l2_reg_dnn, l2_reg_dcn,
                        lr, wd, epochs, patience, seed, device
                        ):
    """
    使用交叉验证来搜索最优的alpha值
    """
    ...
    for alpha in alpha_values:
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(df_unbiased, df_unbiased["is_click"])):
            ...
            model_pre = CDNet(embedding_index=embedding_index,embedding_size=embedding_size,
              dense_feature_num=dense_feature_num,cross_layer_num=cross_layer_num,
            ...
            for epoch in range(epochs):
                ...
                for (train_sparse_inputs_pre, train_dense_inputs_pre, train_labels_pre), (surrogate_sparse_inputs_pre, surrogate_dense_inputs_pre, surrogate_labels_pre) in zip(train_loader,surrogate_loader):
                    ...
                    loss_train_pre = custom_loss_function(y_hat_train_pre, train_labels_pre, model_pre,
                                                         l2_reg_embedding=model_pre.l2_reg_embedding,
                                    l2_reg_dnn=model_pre.l2_reg_dnn,
                                    l2_reg_dcn=model_pre.l2_reg_dcn)
