...
# 初始化早停
early_stopping = EarlyStopping(patience=patience, min_delta=0.001, save_path="Loss_DC_Pessi_fintuning_model_{}_percent.pth".format(int(tuning_percentage*100)) )
...
# 将最佳权重矩阵保存到文件
np.savetxt("best_Loss_DC_Pessi_fintuning_alpha_matrix_tuning_{}_percent.txt".format(int(tuning_percentage*100)), best_alpha_matrix, delimiter=",")

metrics_df.to_csv('metrics_trained_Loss_DC_Pessi_fintuning_{}_percent.csv'.format(int(tuning_percentage*100)), index=False) #依赖于微调数据比例
