...
# 分离标签和特征
train_label = pd.DataFrame(df_train[target])  # 提取标签
train_data = df_train.drop(columns=target[0])  # 提取特征

# 构建训练数据加载器
train_tensor_data = torch.utils.data.TensorDataset(
    torch.from_numpy(np.array(train_data)),  # 转换特征为张量
    torch.from_numpy(np.array(train_label))  # 转换标签为张量
)
train_loader = DataLoader(dataset=train_tensor_data, shuffle=True, batch_size=tuning_batch_size)
...
file_name = 'metrics_supervised_all_model_tuning_{}_percentage.csv'.format(int(tuning_percentage*100)) ## 依赖于微调数据比例！
