...
def get_sparse_emb_dims(feat_sizes, sparse_feature_columns):
    """
    设置稀疏特征的嵌入维度。
    Args:
        feat_sizes (dict): 所有特征的字典，包括特征名称和类别数。
        sparse_feature_columns (list): 稀疏特征的名称列表。
    Returns:
        dict: 每个稀疏特征的嵌入维度字典。
    """
    sparse_emb_dims = {}
    ...
    for feat in sparse_feature_columns:
        num_categories = feat_sizes[feat]
        if num_categories > 1000:
            sparse_emb_dims[feat] = 128
        elif num_categories > 100:
            sparse_emb_dims[feat] = 64
        else:
            sparse_emb_dims[feat] = 32

    return sparse_emb_dims
...
def get_metrics(loader, model, k, device):
    ...
    with torch.no_grad():
        ...
        for sparse_inputs, dense_inputs, labels in loader:
            ...
            loss_test = custom_loss_function(y_hat, y, model,
                                             l2_reg_mlp=model.l2_reg_mlp,
                                             l2_reg_agg=model.l2_reg_agg,
                                             l2_reg_fs=model.l2_reg_fs,
    ...
    # 转换为 numpy 数组
    pred = np.array(pred)
    target = np.array(target, dtype=int)  # 确保 target 是整数类型

    # 确保 target 是二值的，pred 是概率
...
# 定义损失函数
def custom_loss_function(preds, targets, model,
                         l2_reg_mlp=1e-3,l2_reg_agg=5e-4,l2_reg_fs=5e-4,l2_reg_embedding = 5e-4):
    # 计算主损失（例如二值交叉熵损失）
...
def create_data_loaders(batch_size, dataset, sparse_features, dense_features, target, shuffle=True):
    # 稀疏特征：为每个类别特征创建单独的张量
    sparse_inputs = {feat: torch.from_numpy(np.array(dataset[feat])).long() for feat in sparse_features}

    # 稠密特征：将其转换为 PyTorch 的浮点类型张量
    dense_inputs = torch.from_numpy(np.array(dataset[dense_features])).float()

    # 标签：分类问题，将目标变量转换为 long 类型
    labels = torch.from_numpy(np.array(dataset[target])).long()

    # 将稀疏特征的张量按列堆叠，形成 (batch_size, num_sparse_features) 维度的张量
    sparse_inputs_tensor = torch.stack(list(sparse_inputs.values()), dim=1)

    # 构建 TensorDataset，将稀疏特征、稠密特征和标签整合
    dataset = TensorDataset(sparse_inputs_tensor, dense_inputs, labels)
